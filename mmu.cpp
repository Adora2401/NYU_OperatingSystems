...
struct pte_t {
    pte_t() : present(0),referenced(0),modified(0),write_protected(0),pageout(0),file_mapped(0),frameNumber(0),page_index(0){}
...
public:
    frame_t* select_victim_frame() {
        Process cur_procs = proc_arr[frametable[hand]->pid];
        pte_t* pte = cur_procs.page_table[frametable[hand]->page_index];
        while (pte->referenced == 1) {
            pte->referenced = 0;
            hand = (hand + 1) % num_frames;
            cur_procs = proc_arr[frametable[hand]->pid];
...
public:
    frame_t* select_victim_frame() {
        ...
        for (int i = 0; i < num_frames; i++) {
            Process cur_procs = proc_arr[frametable[search_pointer]->pid];
        ...
        if (instr_number > TAU) {
            ...
            for (int i = 0; i < num_frames; i++)
                proc_arr[frametable[i]->pid].page_table[frametable[i]->page_index]->referenced = 0;
...
public:
    frame_t* select_victim_frame() {
        ...
        for (int i =0; i < num_frames; i++){
            ...
            Process cur_proc = proc_arr[frametable[search_pointer]->pid];
    ...
    frame_t* select_victim_frame() {
        ...
        if (temp != -1) {
            ...
            if (reset_count == num_frames){ //reset all frames
                for (int i = 0; i < num_frames; i++) {
                    proc_arr[frametable[search_pointer]->pid].page_table[frametable[search_pointer]->page_index]->referenced = 0;
...
void lineProcess(std::string &line,const std::string comment_str = "#"){
    for(char &c:line){
        if (c == '\t' || c == ',' || c == ';' || c == '\r' || c == '\n')
                    c = ' ';
    }
...
int main(int argc, char ** argv) {
    ...
    while (getline(inputfile, line)) {
        if (line[0]!='#'){ // ALL lines starting with '#' must be ignored
            ...
            if (num_procs == -1) {
                ...
                proc_arr[cur_procs].pid = cur_procs;
                cur_vmas = 0;
            }
                ...
                >> proc_arr[cur_procs].vma_vector.back()->write_protected 
            ...
            if (instr == 'c'){
                ...
                for (int k = 0; k < cur_proc->vma_vector.size(); k++){
                    for (int j = cur_proc->vma_vector[k]->start_vpage; j <= cur_proc->vma_vector[k]->end_vpage; j++){
                        cur_proc->page_table[j]->write_protected = cur_proc->vma_vector[k]->write_protected;
                ...
                if (!pte->present) {                       
                    ...
                    if (!newframe->free) { // need to unmap the victim frame
                        pte_t* prev_pte = proc_arr[newframe->pid].page_table[newframe->page_index];
                        proc_arr[newframe->pid].pstats->unmaps++;
                        cost += 400;
                        if (option_O) cout << " UNMAP " << newframe->pid << ":" << prev_pte->page_index << "\n";
                        
                        if (prev_pte->modified) { // the frame is dirty 
                            if (prev_pte->file_mapped){
                                proc_arr[newframe->pid].pstats->fouts++;
                                cost += 2400;
                                if (option_O) cout << " FOUT\n";
                            }
                            else{
                                proc_arr[newframe->pid].pstats->outs++;
                                cost += 2700;
                                prev_pte->pageout = 1;
                ...
                cout << "EXIT current process " << cur_proc->pid << "\n";
                process_exits++;
                cost += 1250;
                ...
                for (int k = 0; k < cur_proc->vma_vector.size(); k++){
                    for (int i = cur_proc->vma_vector[k]->start_vpage; i <= cur_proc->vma_vector[k]->end_vpage; i++){
                        if (cur_proc->page_table[i]->present) {
                            ...
                            cost += 400;
                            if (option_O) cout << " UNMAP " << cur_proc->pid << ":" << i << "\n";
    ...
    if (option_F) {
        ...
        for (int i = 0; i < num_frames; i++) {
            ...
            cout << frametable[i]->pid << ":" 
            << frametable[i]->page_index;
            if(i!=num_frames-1) cout<<" ";
        }
        cout << "\n";
    }
    ...
    if (option_S) {
        for (int i = 0; i < num_procs; i++) {
            ...
            printf("PROC[%d]: U=%lu M=%lu I=%lu O=%lu FI=%lu FO=%lu Z=%lu SV=%lu SP=%lu\n",proc->pid,pstats->unmaps, pstats->maps, pstats->ins, pstats->outs,pstats->fins, pstats->fouts, pstats->zeros,pstats->segv, pstats->segprot);
        }

        printf("TOTALCOST %lu %lu %lu %llu %lu\n", instr_count, ctx_switches, process_exits, cost, sizeof(pte_t));
    }
